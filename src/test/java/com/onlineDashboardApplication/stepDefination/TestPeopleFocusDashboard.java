		...
		for (int i = 0; i < tablerows.size(); i++) {
			...
			for (int j = 0; j < columnrow.size(); j++) {
				if (columnrow.get(0).getText().equals("13")) {
					...
					Assert.assertEquals(columnrow.get(2).getText(), "Consultant");
					Assert.assertEquals(columnrow.get(3).getText(), "Sujay Rahane");
					Assert.assertEquals(columnrow.get(4).getText(), "Yes");
					Assert.assertEquals(columnrow.get(5).getText(), "09-08-2020");
					Assert.assertEquals(columnrow.get(6).getText(), "Appreciation");
					Assert.assertEquals(columnrow.get(7).getText(), "Good Content");
					Assert.assertEquals(columnrow.get(8).getText(), "escalation content");
					Assert.assertEquals(columnrow.get(9).getText(), "Nice Work");
				}
			}
		}
	}
